...
This BTNOMB submission is a compact MVP optimized for reviewer validation. It is functional locally, packaged for Docker, and now includes health/readiness endpoints plus a production smoke-test script. The following items should still be hardened before high-volume production use.
...
- Live deployment verification is supported by `/healthz`, `/readyz`, Docker Compose health checks, Render metadata, and `scripts/production_smoke_test.py`; this package does not commit provider-specific credentials or a permanent hosted URL.
